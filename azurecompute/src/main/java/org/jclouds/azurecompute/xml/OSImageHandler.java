...
final class OSImageHandler extends ParseSax.HandlerForGeneratedRequestWithResult<OSImage> {
   ...
   @Override public OSImage getResult() {
      OSImage result = OSImage.create(name, ImmutableList.copyOf(locations), affinityGroup, label, description, category, os, mediaLink,
              logicalSizeInGB, ImmutableList.copyOf(eulas), imageFamily, publishedDate, iconUri, smallIconUri,
              privacyUri, pricingDetailLink, recommendedVMSize, isPremium, showInGui, publisherName);
