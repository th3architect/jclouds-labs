...
public class CatalogApiLiveTest extends BaseVCloudDirectorApiLiveTest {
   ...
   protected void setupRequiredApis() {
      ...
      adminCatalogApi = api.getAdminCatalogApi();
   ...
   protected void tearDownContext() {
      if (media != null) {
         try {
            Task remove = api.getMediaApi().remove(media.getId());
      ...
      if (createdByTest) {
         ...
         if (adminCatalog != null) {
            try {
               api.getAdminCatalogApi().remove(adminCatalog.getId());
            } catch (Exception e) {
   ...
   public void testGetCatalogMetadata() {
      Metadata catalogMetadata = api.getMetadataApi(catalogUrn).get();
   ...
   public void testGetCatalogMetadataValue() {
      ...
      Task mergeCatalogMetadata = api.getMetadataApi(catalogUrn)
            .putAll(ImmutableMap.of("KEY", "MARMALADE"));
      assertTaskSucceedsLong(mergeCatalogMetadata);

      Metadata catalogMetadata = api.getMetadataApi(catalogUrn).get();

      String metadataValue = api.getMetadataApi(catalogUrn).get("KEY");
   ...
   public void testGetCatalogItemMetadata() {
      Metadata catalogItemMetadata = api.getMetadataApi(catalogItem.getId()).get();
   ...
   public void testMergeCatalogItemMetadata() {
      Metadata before = api.getMetadataApi(catalogItem.getId()).get();

      Task mergeCatalogItemMetadata = api.getMetadataApi(catalogItem.getId()).putAll(
      ...
      Metadata mergedCatalogItemMetadata = api.getMetadataApi(catalogItem.getId()).get();
      ...
      String keyMetadataValue = api.getMetadataApi(catalogItem.getId()).get("KEY");
      assertEquals(keyMetadataValue, "MARMALADE", "The Value of the MetadataValue for KEY should have changed");

      String newKeyMetadataValue = api.getMetadataApi(catalogItem.getId()).get("VEGIMITE");
   ...
   public void testGetCatalogItemMetadataValue() {
      String metadataValue = api.getMetadataApi(catalogItem.getId()).get("KEY");
   ...
   public void testSetCatalogItemMetadataValue() {
      ...
      Task setCatalogItemMetadataValue = api.getMetadataApi(catalogItem.getId()).put("KEY", "NEW");
      ...
      String editedMetadataValue = api.getMetadataApi(catalogItem.getId()).get("KEY");
   ...
   public void testRemoveCatalogItemMetadataValue() {
      Task removeCatalogItemMetadataValue = api.getMetadataApi(catalogItem.getId()).remove("KEY");
      ...
      String removed = api.getMetadataApi(catalogItem.getId()).get("KEY");
      assertNull(removed);
   }
