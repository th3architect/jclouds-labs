...
public class MiscApiLiveTest extends BaseDockerApiLiveTest {
   ...
   public void testVersion() {
      assertEquals(api().getVersion().getApiVersion(), API_VERSION);
      assertEquals(api().getVersion().getVersion(), VERSION);
      assertEquals(api().getVersion().getGitCommit(), GIT_COMMIT);
      assertEquals(api().getVersion().getGoVersion(), GO_VERSION);
   }
   ...
   public void testBuildImageFromDockerfile() throws IOException, InterruptedException, URISyntaxException {
      ...
      InputStream buildImageStream = api().build(new File(Resources.getResource("SimpleDockerfile").toURI()), options);
   ...
   protected void tearDown() {
      if (imageId != null) {
         consumeStream(api.getImageApi().deleteImage(imageId), true);
      }
   }
